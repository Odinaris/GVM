function [fi,run_time] = embed(img_name,secret,feasible_solutions,num_slt_pks)
t1 = clock;
%% Read JPEG image.
...
while flag <= blk_num
    [ac_pos(flag),dc_app_len(flag,1),dc_code{flag}] = parse_dc(bin_sos, table_huff_dc, dc_pos(flag));
    [dc_pos(flag+1), ac_code{flag}] = parse_ac(bin_sos, table_huff_ac, ac_pos(flag));
    flag = flag + 1;
end
%% Parse the VLCs.
...
G_opt = get_gvm_relationship( vlc, feasible_solutions, payload, 6, num_slt_pks);
    ...
    for j = 1:num_zrv
        ...
        if ind ~= 0
            ...
            if idx_secret+len_bit-1 > payload
                secret(payload+1:idx_secret+len_bit-1) = ones(idx_secret+len_bit-1-payload,1);
            end
...
jpg_ecs = gen_ecs(bin_sos,dc_code,mdf_ac_code,blk_num);
stego_jpg = [jpg_header;jpg_ecs];
fid = fopen('stego.jpg', 'w+');
