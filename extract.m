function [secret,run_time] = extract(img_name,payload,table_huff_ac_default)
t1 = clock;
%% Read an image.
...
while flag <= blk_num
    [ac_pos(flag),~,dc_code{flag}] = parse_dc(bin_ecs, table_huff_dc, dc_pos(flag));  
...
jpg_ecs = gen_ecs(bin_ecs,dc_code,rst_ac_code,blk_num);
rst_jpg = [jpg_header;jpg_ecs];
fid = fopen('restored.jpg', 'w+');
fwrite(fid,rst_jpg,'uint8');
